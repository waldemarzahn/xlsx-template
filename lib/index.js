...
//
    ...
    */
    ...
    Workbook.prototype.fixColumnsWidths = function(sheetName) {
        var template = this;
        var sheet = template.loadSheet(sheetName);

        //get all the cells (and contents) in the worksheet
        var cells = sheet.root.findall('./sheetData/row/c').map(root => {
            var r = root.find('v');
            if (r != null) {
                return {
                    root,
                    ref: template.splitRef(root.attrib.r),
                    stringIndex: parseInt(root.find('v').text),
                    string: (root.attrib.t === 's')? template.sharedStrings[parseInt(root.find('v').text)]: root.find('v').text
                };
            }
        });

        //group cells by column
        var cols = cells.reduce((index, cell) => {
            if (cell) {
                index[cell.ref.col] = [...index[cell.ref.col] || [], cell];
            }
            return index;
        }, {});

        var widths = Object.keys(cols).map((col) => {
            return {
                col,
                colNr: template.charToNum(col),
                width: cols[col].reduce((width, cell) => {
                    return Math.max(width, cell.string.length);
                }, 0)
            }
        });

        var colsEl = sheet.root.find('./cols');

        colsEl.getchildren().forEach(child => colsEl.remove(child));
        widths.forEach(def => {
            var col = etree.SubElement(colsEl, 'col');
            col.set('min', def.colNr);
            col.set('max', def.colNr);
            col.set('width', def.width + 2);
